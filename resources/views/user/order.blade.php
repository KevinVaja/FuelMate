          ...
          <div class="modal-dialog modal-dialog-centered">
            <div class="modal-content">
              <div class="modal-header">
                <h5 class="modal-title" id="otpModalTitle">Demo OTP Verification</h5>
                <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
              </div>
              <div class="modal-body text-center">
                <p class="text-muted mb-3">Clicking Place Order generates a demo OTP. Enter that 6-digit OTP below to confirm the order.</p>
                <div class="alert alert-primary d-none py-2" id="otpDemoNotice">
                  <div class="small text-uppercase fw-semibold">Generated Demo OTP</div>
                  <div class="fs-4 fw-bold mb-0" id="otpDemoCode">------</div>
                </div>
                <div id="otpStatus" class="text-muted small mb-3">We are preparing your OTP.</div>
                <input type="text" id="otpInput" class="form-control text-center fw-bold" placeholder="Enter OTP" inputmode="numeric" maxlength="6" autocomplete="one-time-code">
                <div id="otpError" class="text-danger small mt-2 d-none">Invalid OTP</div>
              </div>
              <div class="modal-footer justify-content-between">
                <button type="button" class="btn btn-outline-secondary" data-bs-dismiss="modal">Cancel</button>
                  <button type="button" class="btn btn-primary" id="verifyOtpBtn">Verify OTP &amp; Place Order</button>
              </div>
            </div>
          </div>
        </div>
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  const otpDemoNotice = document.getElementById('otpDemoNotice');
  ...
  const setOrderSubmitState = (isBusy) => {
    ...
    placeOrderButton.innerHTML = isBusy
      ? '<span class="spinner-border spinner-border-sm me-2" aria-hidden="true"></span>Generating Demo OTP...'
  ...
  const resetOtpModalState = () => {
    otpInput.value = '';
    hideOtpError();
    otpStatus.textContent = 'We are preparing your demo OTP.';
    otpDemoCode.textContent = '------';
    otpDemoNotice.classList.add('d-none');
  ...
  form.addEventListener('submit', async (event) => {
    ...
    try {
      ...
      otpStatus.textContent = data.message ?? 'OTP generated successfully.';
      otpDemoCode.textContent = data.otp ?? '------';
      otpDemoNotice.classList.toggle('d-none', !data.otp);
      otpModal.show();
    } catch (error) {
      if (getSelectedMode() === 'live_gps') {
        liveStatus.textContent = error.message || 'Location is ready, but OTP could not be sent. Please try again.';
      } else {
        renderMapState(error.message || 'Location is ready, but OTP could not be sent. Please try again.');
      }
    } finally {
      requestingOtp = false;
  ...
  const verifyOtp = async () => {
    ...
    try {
      ...
      if (!response.ok || !data?.status) {
        showOtpError(data?.message ?? 'Unable to verify OTP right now. Please try again.');
        setOtpVerifyState(false);
        return;
      }
      ...
      showOtpError(error.message || 'Unable to verify OTP right now. Please try again.');
      setOtpVerifyState(false);
    }
  };
