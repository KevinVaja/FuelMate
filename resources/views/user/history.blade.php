...
<div class="card">
  <div class="table-responsive">
    <table class="table table-hover mb-0">
      <thead class="table-light"><tr><th>#</th><th>Fuel</th><th>Qty</th><th>Address</th><th>Total</th><th>Payment</th><th>Status</th><th>Date</th><th colspan="2">Actions</th></tr></thead>
      <tbody>
        ...
        <tr>
          ...
          <td colspan="2">
            <div class="d-flex gap-2 flex-wrap">
              <a href="{{ route('user.track', $order->id) }}" class="btn btn-sm btn-outline-primary">Track</a>
              @if($order->billing)
              <a href="{{ route('orders.invoice', $order->id) }}" class="btn btn-sm btn-outline-secondary">View Invoice</a>
              <a href="{{ route('orders.invoice.download', $order->id) }}" class="btn btn-sm btn-primary">Download</a>
              @endif
            </div>
          </td>
        </tr>
