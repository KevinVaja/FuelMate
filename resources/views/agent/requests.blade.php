...
@else
  <div class="d-flex flex-column gap-3">
    ...
    @php $mapsUrl = $request->googleMapsDirectionsUrl($agent->current_lat, $agent->current_lng); @endphp
    <div class="card">
      <div class="card-body">
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  const syncLocation = () => {
    ...
    navigator.geolocation.getCurrentPosition(async (position) => {
      ...
      try {
        ...
        if (gpsStatus) {
          gpsStatus.textContent = `Live location synced at ${new Date().toLocaleTimeString()}. Refresh to see updated ETA.`;
        }
      } catch (error) {
        if (gpsStatus) {
      ...
      if (gpsStatus) {
        gpsStatus.textContent = 'Allow location access to see accurate customer distance and ETA.';
      }
    }, {
  ...
  syncLocation();
  window.setInterval(syncLocation, 15000);
});
</script>
@endsection
