...
@else
@php $mapsUrl = $order->googleMapsDirectionsUrl(auth()->user()->agent?->trackingLatitude(), auth()->user()->agent?->trackingLongitude()); @endphp
<div class="row g-4">
  <div class="col-lg-6">
    <div class="card mb-4">
      ...
      <div class="card-body">
        ...
        <div class="row g-2">
          ...
          <div class="col-6"><small class="text-muted">Status</small><span class="badge badge-status-{{ $order->status }}">{{ $order->statusLabel() }}</span></div>
  ...
  <div class="col-lg-6">
    <div class="card">
      ...
      <div class="card-body">
        ...
        <div class="p-4 bg-primary bg-opacity-10 rounded-3 border border-primary border-opacity-25 mb-3">
          <p class="mb-1 text-muted small">Current Status:</p>
          <div class="fw-bold fs-5 mb-3">{{ $order->statusLabel() }}</div>
          <form method="POST" action="{{ route('agent.status', $order->id) }}">
            @csrf
            <input type="hidden" name="current_lat" class="agent-status-lat">
            <input type="hidden" name="current_lng" class="agent-status-lng">
            <button type="submit" class="btn btn-primary btn-lg w-100"><i class="fas fa-key me-2"></i>Start Handoff Verification</button>
          </form>
        </div>
        <div class="small text-muted">Start this only when the customer is ready to confirm the delivery with a PIN or QR.</div>
        ...
        <div class="p-4 bg-primary bg-opacity-10 rounded-3 border border-primary border-opacity-25 mb-3">
          ...
          <div class="alert alert-warning mb-3">
            <div class="fw-bold small mb-1">Waiting for customer handoff proof</div>
            <div class="small mb-0">Ask the customer to show the 6-digit delivery PIN or the FuelMate QR from their tracking page, then confirm the handoff here.</div>
          </div>
          <form method="POST" action="{{ route('agent.status', $order->id) }}">
            @csrf
            <input type="hidden" name="current_lat" class="agent-status-lat">
            ...
            <div class="mb-3">
              <div class="d-flex justify-content-between align-items-center gap-2 mb-2">
                <label class="form-label fw-600 mb-0">Customer QR</label>
                <button type="button" class="btn btn-outline-secondary btn-sm" id="startQrScanBtn">Scan QR</button>
              </div>
              <div id="agentQrScanStatus" class="small text-muted mb-2">You can scan the customer's FuelMate QR or enter the 6-digit PIN manually.</div>
              <div id="agentQrReader" class="rounded border p-2 d-none bg-light"></div>
            </div>
            <div class="mb-3">
              <label class="form-label fw-600">Customer Delivery PIN</label>
              <input type="text" name="delivery_otp" id="deliveryOtpInput" class="form-control form-control-lg text-center fw-bold" maxlength="6" placeholder="Enter 6-digit PIN" inputmode="numeric">
              <input type="hidden" name="delivery_handoff_token" id="deliveryHandoffTokenInput">
            </div>
            <button type="submit" class="btn btn-primary btn-lg w-100"><i class="fas fa-check-circle me-2"></i>Verify Handoff & Mark as Delivered</button>
          </form>
        </div>
        <div class="small text-muted">This handoff step confirms that the fuel has actually reached the customer before the order moves to delivered.</div>
...
@endsection
@section('scripts')
@if($order && $order->status === 'otp_verification')
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  const qrScanButton = document.getElementById('startQrScanBtn');
  const qrReader = document.getElementById('agentQrReader');
  const qrScanStatus = document.getElementById('agentQrScanStatus');
  const deliveryOtpInput = document.getElementById('deliveryOtpInput');
  const deliveryHandoffTokenInput = document.getElementById('deliveryHandoffTokenInput');
  let qrScanner = null;
  ...
  const stopQrScanner = async () => {
    if (qrScanner && qrScannerActive) {
      try {
        ...
        // Ignore stop failures caused by an already-closed camera stream.
      }
    }
    ...
    if (qrScanButton) {
      qrScanButton.textContent = 'Scan QR';
    }
  };
  ...
  const handleScannedQr = async (decodedText) => {
    ...
    if (segments.length !== 3 || segments[0] !== 'FM-HANDOFF') {
      if (qrScanStatus) {
        qrScanStatus.textContent = 'This QR code is not a FuelMate delivery handoff code.';
      }
      return;
    }

    if (segments[1] !== '{{ $order->id }}') {
      if (qrScanStatus) {
        qrScanStatus.textContent = 'This QR belongs to a different order.';
      }
      return;
    }

    if (deliveryHandoffTokenInput) {
      deliveryHandoffTokenInput.value = segments[2];
    }

    if (deliveryOtpInput) {
      deliveryOtpInput.value = '';
    }

    if (qrScanStatus) {
      qrScanStatus.textContent = 'Customer QR captured successfully. You can submit the delivery confirmation now.';
    }
  ...
  if (deliveryOtpInput && deliveryHandoffTokenInput) {
    deliveryOtpInput.addEventListener('input', () => {
      if (deliveryOtpInput.value.trim() !== '') {
        deliveryHandoffTokenInput.value = '';
      }
    });
  }

  if (qrScanButton && qrReader) {
    qrScanButton.addEventListener('click', async () => {
      if (qrScannerActive) {
        await stopQrScanner();
        if (qrScanStatus) {
          qrScanStatus.textContent = 'QR scanning stopped. You can still enter the PIN manually.';
        }
        return;
      }
      ...
      if (typeof Html5Qrcode === 'undefined') {
        if (qrScanStatus) {
          qrScanStatus.textContent = 'The QR scanner could not load. Enter the 6-digit PIN manually.';
        }
        return;
      }
      ...
      qrReader.innerHTML = '';
      qrScanner = new Html5Qrcode('agentQrReader');
      ...
      try {
        ...
        qrScanButton.textContent = 'Stop Scan';

        if (qrScanStatus) {
          qrScanStatus.textContent = 'Point your camera at the customer QR.';
        }
      } catch (error) {
        await stopQrScanner();

        if (qrScanStatus) {
          qrScanStatus.textContent = 'Unable to start the QR scanner. Enter the 6-digit PIN manually if needed.';
        }
      }
    });
  }
