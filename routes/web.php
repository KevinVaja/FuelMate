...
Route::post('/register/email-otp/send', [AuthController::class, 'sendRegistrationOtp'])
    ->middleware(['guest', 'throttle:6,1'])
    ->name('register.email_otp.send');
Route::post('/register/email-otp/verify', [AuthController::class, 'verifyRegistrationOtp'])
    ->middleware(['guest', 'throttle:6,1'])
    ->name('register.email_otp.verify');
Route::post('/register', [AuthController::class, 'register']);
Route::post('/logout', [AuthController::class, 'logout'])->name('logout');

Route::post('/send-otp', [FuelRequestController::class,'sendOtp'])
    ->middleware(['auth', 'role:user', 'throttle:6,1'])
    ->name('user.send.otp');
Route::post('/verify-otp', [FuelRequestController::class,'verifyOtp'])
    ->middleware(['auth', 'role:user', 'throttle:6,1'])
    ->name('user.verify.otp');

Route::middleware(['auth', 'role:agent', 'agent.approved', 'throttle:60,1'])->prefix('api')->group(function () {
    Route::post('/agent/update-location', [AgentController::class, 'updateLocation'])->name('api.agent.location.update');
});

Route::middleware(['auth', 'role:user', 'throttle:60,1'])->prefix('api')->group(function () {
    Route::get('/order/{id}/track', [UserController::class, 'trackData'])->name('api.order.track');
});
...
Route::middleware(['auth', 'role:user'])->get('/orders/{id}/invoice', [BillingController::class, 'invoice'])
    ->name('orders.invoice');
Route::middleware(['auth', 'role:user'])->get('/orders/{id}/invoice/download', [BillingController::class, 'downloadInvoice'])
...
Route::middleware(['auth', 'role:admin'])->prefix('admin')->group(function () {
    ...
    Route::get('/agent-coverage', [AdminController::class, 'agentCoverage'])->name('admin.agent_coverage');
    Route::get('/agent-coverage/data', [AdminController::class, 'agentCoverageData'])->name('admin.agent_coverage.data');
    Route::patch('/agents/{id}/pump-location', [AdminController::class, 'updateAgentPumpLocation'])->name('admin.agents.pump_location.update');
