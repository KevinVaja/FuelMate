...
class BillingController extends Controller
{
    public function estimate(Request $request, BillingService $billingService): JsonResponse
    {
        ...
        if (! $billingService->isDistanceWithinServiceRange($distanceKm)) {
            return response()->json([
                'status' => false,
                'message' => 'Delivery distance must be within 25 km.',
            ], 422);
        }
    ...
    {
        $order = $this->findInvoiceOrder($id);

        return view('user.invoice', compact('order'));
    }

    public function downloadInvoice(int $id, InvoicePdfService $invoicePdfService)
    {
        $order = $this->findInvoiceOrder($id);
        $filename = 'fuelmate-invoice-order-' . $order->displayOrderNumber() . '-' . $order->id . '.pdf';

        return response($invoicePdfService->render($order), 200, [
    ...
    private function findInvoiceOrder(int $id): FuelRequest
    {
