...
class UserController extends Controller {
    ...
    public function placeOrder(
        ...
        if (! $this->hasFreshOrderOtpVerification($request)) {
            return back()
                ->withInput()
                ->with('error', 'Please verify the OTP before placing your order.');
        }
        ...
        if (! $billingService->isDistanceWithinServiceRange($distanceKm)) {
            return back()
                ->withInput()
                ->with('error', 'Delivery distance must be between 0.1 and 25 km.');
        }
        ...
        $request->session()->forget(self::OTP_VERIFIED_AT_SESSION_KEY);
    ...
    private function hasFreshOrderOtpVerification(Request $request): bool {
        $verifiedAt = $request->session()->get(self::OTP_VERIFIED_AT_SESSION_KEY);
